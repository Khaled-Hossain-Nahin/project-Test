...
int predictBallY(int startX, int startY, int vx, int vy, int targetX)
{
    ...
    // Stop when the ball reaches OR crosses the target X.
    // Using x != targetX would loop forever when vx is greater than 1
    // and targetX is not exactly reachable (for example 40 -> 45 -> ... -> 80).
...
void aiController()
{
    ...
    {
        // Simple AI follows the ball's current position.
        targetY = Bally - paddle_height / 2;
    }
    else
    {
        // Predictive AI is intentionally imperfect. Even on Hard it can
        // make a small prediction error, so it is not unbeatable.
        ...
        if (rand() % 100 < errorChance)
            targetY += (rand() % (2 * errorRange + 1)) - errorRange;
    }
...
void main_logic()
{
    // Keep the old X position so fast balls cannot skip over a paddle
    // between two frames.
    int oldBallX = Ballx;
...
void algorithm_demo()
{
    ...
    cout << "   - Predicts the ball path, but intentionally makes occasional prediction errors.\n\n";
